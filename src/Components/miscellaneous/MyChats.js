...
export default function MyChats({ fetchAgain }) {
  ...
  const fetchChats = async () => {
    try {
      ...
      const { data } = await axios.get(`${process.env.REACT_APP_URL}/chat`, config);
      setChats(data);
    } catch (error) {
      toast({
        title: "Error Occured",
            ...
            {chats.map((chat) => (
              ...
              >
                ...
                <Image
                  borderRadius="full"
                  boxSize='30px'
                  src={!chat.isGroupChat?getSenderImage(loggedUser,chat.users):'https://res.cloudinary.com/dycxuzuon/image/upload/v1692508558/166258_nqoutt.png'}
